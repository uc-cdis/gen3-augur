...
class TestSubcommandParseGenbank(unittest.TestCase):
    def test_parse_bg(self):
        ...
        with IO.change_dir(dir_path):
            self.assertIsNone(ParseGenBank.parse_bg('data/test-genbank-rawbg/EU371562.gb', logger))
            (meta, seq) = ParseGenBank.parse_bg('data/test-genbank-rawbg/MT027062.gb', logger)
